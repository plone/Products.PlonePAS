...
class TestPAS(PlonePASTestCase):
    def afterSetUp(self):
    ...
    def testSimpleSearch(self):
        mship = self.portal.portal_membership
        mship.addMember("user1", "secret", ["Manager",], [])
        mship.addMember("user2", "secret", ["Manager",], [])

        self.assertEqual(
            self.plugin.enumerateUsers(id="user1",
                    exact_match=False),
            () )
